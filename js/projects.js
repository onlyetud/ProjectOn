document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".toggle-lots").forEach(function (btn) {
    btn.addEventListener("click", function (e) {
      ...
      if (!projectId) return;
      var lotsRow = document.querySelector(
        '.lots-row[data-parent="' + projectId + '"]',
      );
      if (!lotsRow) return;
      var isHidden = window.getComputedStyle(lotsRow).display === "none";
      // hide all others
      document.querySelectorAll(".lots-row").forEach(function (r) {
        r.style.display = "none";
      });
      // reset all chevrons to down
      document.querySelectorAll(".toggle-lots i").forEach(function (ic) {
      ...
      if (map) {
        if (lat && lng) {
          var src =
            ...
            "&z=15&output=embed";
          // create iframe without inline styles; rely on CSS class
          var iframe = document.createElement("iframe");
          iframe.src = src;
          iframe.setAttribute("loading", "lazy");
          iframe.className = "pd-iframe";
          while (map.firstChild) map.removeChild(map.firstChild);
          map.appendChild(iframe);
        } else {
          // use existing muted styles rather than inline color
          while (map.firstChild) map.removeChild(map.firstChild);
          var no = document.createElement("div");
          no.className = "muted";
          no.textContent = "No coordinates available";
          map.appendChild(no);
        }
      }
